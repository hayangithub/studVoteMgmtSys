		...
		<div class="container">
			<div class="row">
				<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
					...
					action="${pageContext.request.contextPath}/edit-position-done" method="POST" modelAttribute="position">
					...
					<c:if test="${errorHandler.errorCode eq '0008' }">
                        <div class="basic-tb-hd">
                            <p class="error"> <c:out value="${positionName}"/>  <c:out value="${errorHandler.errorMessage}"/> </p>
                        </div>
                    </c:if>
                    <!-- Validation Check -->
											...
											<div class="nk-int-st">
												<div class="bootstrap-select fm-cmp-mg">
													<select class="selectpicker" name="status">
														<option value="A"
															<c:if test="${position.status eq 'A'}">
													  selected
													</c:if>>Active</option>
														<option value="I"
															<c:if test="${position.status eq 'I'}">
													  selected
